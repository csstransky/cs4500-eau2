...
public:
  ...
  void readInput() {
    ...
    if (index == 0) {
      pln("Reading...");
      projects = DataFrame::from_file(PROJ, pK.clone(), &kv);
      p("    ").p(projects->nrows()).pln(" projects");
      users = DataFrame::from_file(USER, uK.clone(), &kv);
      p("    ").p(users->nrows()).pln(" users");
      commits = DataFrame::from_file(COMM, cK.clone(), &kv);
       p("    ").p(commits->nrows()).pln(" commits");
       // This dataframe contains the id of Linus.
       delete DataFrame::from_scalar(new Key("users-0-0"), &kv, LINUS);
    } else {
  ...
  *  projects, and the users added in the previous round. */
  ...
  void merge(Set& set, char const* name, int stage) {
	...
	delete delta;
      ...
      delete DataFrame::from_rower(&k, &kv, "I", writer);
    } else {

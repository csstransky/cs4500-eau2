...
public:
  ...
  Column* get(size_t index) { return dynamic_cast<Column*>(ObjectArray::get(index)); }
  ...
  /* Throws an error if not found or out of range or no elements in array*/
  Column* remove(size_t index) { return dynamic_cast<Column*>(ObjectArray::remove(index)); }
